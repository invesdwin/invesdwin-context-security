...
class Blake2b {
    ...
    public void init(final int digestSize) {
    ...
    void update(final byte[] message) {
        if (message == null) {
            return;
        }
     ...
     */
    void update(final byte[] message, final int offset, final int len) {
    ...
    /**
     * close the digest, producing the final digest value. The doFinal call leaves the digest reset. Key, salt and
     * personal string remain.
     *
     * @param out
     ...
     */
    void doFinal(final byte[] out, final int outOffset) {
        ...
        for (int i = 0; i < chainValue.length && (i * 8 < digestLength); i++) {
            final byte[] bytes = Utils.longToLittleEndian(chainValue[i]);
    ...
    private void compress(final byte[] message, final int messagePos) {
    ...
    private void functionG(final long m1, final long m2, final int posA, final int posB, final int posC,
            final int posD) {
