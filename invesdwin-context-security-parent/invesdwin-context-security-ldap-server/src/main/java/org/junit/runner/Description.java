 ...
 * atomic (a single test) or compound (containing children tests). <code>Descriptions</code> are used to provide
 * feedback about the tests that are about to run (for example, the tree view visible in many IDEs) or tests that have
 * been run (for example, the failures view).
 * <p>
 * <code>Descriptions</code> are implemented as a single class rather than a Composite because they are entirely
 * informational. They contain no logic aside from counting their tests.
 * <p>
 * In the past, we used the raw {@link junit.framework.TestCase}s and {@link junit.framework.TestSuite}s to display the
 * tree of tests. This was no longer viable in JUnit 4 because atomic tests no longer have a superclass below
 * {@link Object}. We needed a way to pass a class and name together. Description emerged from this.
 *
...
public final class Description implements Serializable {
    ...
    /**
     * Create a <code>Description</code> named <code>name</code>. Generally, you will add children to this
     ...
     *            the name of the <code>Description</code>
     * @param uniqueId
     *            an arbitrary object used to define uniqueness (in {@link #equals(Object)}
     * @param annotations
     *            meta-data about the test, for downstream interpreters
     * @return a <code>Description</code> named <code>name</code>
     */
    public static Description createSuiteDescription(final String name, final Serializable uniqueId,
     ...
     * @param name
     *            the name of the test (a method name for test annotated with {@link org.junit.Test})
     * @param annotations
     *            meta-data about the test, for downstream interpreters
     * @return a <code>Description</code> named <code>name</code>
     */
    public static Description createTestDescription(final String className, final String name,
            final Annotation... annotations) {
    ...
    /**
     * Create a <code>Description</code> of a single test named <code>name</code> in the class <code>clazz</code>.
     * Generally, this will be a leaf <code>Description</code>. (This remains for binary compatibility with clients of
     * JUnit 4.3)
     *
     * @param clazz
     *            the class of the test
     ...
     *
     * @param testClass
     *            A not null {@link Class} containing tests
     * @param annotations
     *            meta-data about the test, for downstream interpreters
     * @return a <code>Description</code> of <code>testClass</code>
     */
    public static Description createSuiteDescription(final Class<?> testClass, final Annotation... annotations) {
    ...
    /**
     * Describes a step in the test-running mechanism that goes so wrong no other description can be used (for example,
     * an exception thrown from a Runner's constructor
     */
    public static final Description TEST_MECHANISM = new Description(null, "Test mechanism");

    /*
     * We have to use the f prefix until the next major release to ensure serialization compatibility. See
     * https://github.com/junit-team/junit4/issues/976
     */
    private final Collection<Description> fChildren = Collections.synchronizedList(new LinkedList<>());
    ...
    private Description(final Class<?> testClass, final String displayName, final Serializable uniqueId,
            final Annotation... annotations) {
    ...
    public int testCount() {
        ...
        int result = 0;
        for (final Description child : fChildren) {
    ...
    /**
     * @return a copy of this description, with no children (on the assumption that some of the children will be added
     *         back)
     */
    ...
    public <T extends Annotation> T getAnnotation(final Class<T> annotationType) {
        for (final Annotation each : fAnnotations) {
    ...
    private String methodAndClassNamePatternGroupOrDefault(final int group, final String defaultString) {
        final Matcher matcher = METHOD_AND_CLASS_NAME_PATTERN.matcher(toString());
